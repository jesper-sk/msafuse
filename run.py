...
def run_flat(file_struct, bounds_module, labels_module, frame_times, config,
    ...
    # Different boundary and label algorithms
        ...
        if bounds_module is not None:
            ...
            try:
                ...
                # est_times & est_labels zijn goed

                # print("frame_times in run.py:147:", frame_times.shape)

                est_idxs = io.align_times(est_times, frame_times)
                # print("est_idxs in run_flat:148:", est_idxs.shape)
                if est_idxs[0] != 0:
    ...
    if isinstance(features, list):
        ...
        est_times, est_labels = utils.process_segmentation_level(
            est_idxs, est_labels, features[0].shape[0], frame_times,
            config["features"].dur)
    else:
        # Make sure the first and last boundaries are included
        est_times, est_labels = utils.process_segmentation_level(
...
def run_algorithms(file_struct, boundaries_id, labels_id, config,
    ...
    if config["features"].is_fused():
        framecount = config["features"].features[0].shape[0]
    else:
        framecount = config["features"].features.shape[0]

    if framecount <= msaf.config.minimum_frames:
...
def process_track(file_struct, boundaries_id, labels_id, config,
    ...
    est_labels: np.array
        List of all the labels associated segments.
    """
    logging.info("Segmenting %s" % ('/'.join(file_struct.audio_file.split('\\')[-3:])))
    ...
    config["features_default"] = None if config["feature_default"] is None else \
        Features.select_features(
            config["feature_default"], file_struct, config["annot_beats"], 
            config["framesync"])
    ...
    # Save
    logging.info("Writing results in: %s" % ('/'.join(file_struct.est_file.split('\\')[-3:])))
...
def process(in_path, annot_beats=False, feature="pcp", framesync=False,
    ...
    feat_def: str
        String representing the feature to be used if a fused feature is chosen 
        as `feature`, but the chosen algorithm doesn't support fused features
    hier : bool
    ...
    if isinstance(feat_def, list):
        raise TypeError("No fused features allowed as default feature")
    ...
    if os.path.isfile(in_path):
        ...
        config["features_default"] = None if \
            config["feature_default"] is None else \
                Features.select_features(config["feature_default"], file_struct, 
...
def compute_features(in_path, n_jobs=4):
    os.system("cls")
    if not os.path.isfile(in_path):
        file_structs = io.get_dataset_files(in_path)
        return Parallel(n_jobs=n_jobs)(delayed(Features.compute_file_features)(file_struct, idx) for \
            file_struct, idx in zip(file_structs[:], range(len(file_structs))))
    else:
        file_struct = msaf.io.FileStruct(in_path)
        return compute_file_features(file_struct)
