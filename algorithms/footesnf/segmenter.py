...
def compute_ssm(X, metric="seuclidean"):
    ...
    if np.any(np.isnan(D)):
        logging.warn("NaN in SSM detected!")
...
class Segmenter(SegmenterInterface):
    ...
    def processFlat(self):
        ...
        for i in range(len(X)):
            X[i] = msaf.utils.normalize(X[i], norm_type=self.config["norm_type"])
            X[i] = median_filter(X[i], M=self.config["m_median"])
        ...
        est_idxs = np.concatenate(([0], est_idxs, [S.shape[0] - 1]))
