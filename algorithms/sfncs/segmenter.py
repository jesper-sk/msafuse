    ...
    def process_ncsum(self):
        ...
        for F in F_list:
            ...
            if F.shape[0] > 20:
                ...
                # If framesync, make F smaller but keep linear growth
                if self.framesync:
                    red = 0.1
                    F_copy = np.copy(F)
                    F = librosa.util.utils.sync(
                        F.T, np.linspace(0, F.shape[0], num=int(F.shape[0] * red)),
                        pad=False).T
