...
class Segmenter(SegmenterInterface):
    ...
    def processFlat(self):
        ...
        F_list = self._preprocess(valid_features=["fuse"])
        ncs = []

        # Compute novelty curve for each feature matrix
        for F in F_list:
            # print(F)
            # print('\n')
            # Normalize
            ...
            # plt.figure(); plt.plot(nc); plt.show()

            ncs.append(nc)

        # element-wise sum to get fused novelty curve
        fused_nc = np.sum(ncs, axis=0)
        
        # print(ncs)

        # normalize
        fused_nc += fused_nc.min()
        fused_nc /= fused_nc.max()

        # plt.figure(); plt.plot(fused_nc); plt.show()
    
        # Find peaks in the novelty curve
        est_idxs = pick_peaks(fused_nc, L=self.config["L_peaks"])

        # Add first and last frames
        est_idxs = np.concatenate(([0], est_idxs, [F_list[0].shape[0] - 1]))
