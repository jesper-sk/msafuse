...
class Features(six.with_metaclass(MetaFeatures)):
    ...
    def __init__(self, file_struct, sr, hop_length, feat_type):
        ...
        self._ann_beatsync_features = None  # Annotated Beat-sync features, ground-truth
    ...
    def _compute_framesync_times(self):
        ...
        if isinstance(self._framesync_features, list):
            self._framesync_times = \
            [   librosa.core.frames_to_time(np.arange(feat.shape[0]), 
                self.sr, self.hop_length) for \
                feat in self._framesync_features
            ]
        else:
    ...
    def features(self):
        ...
        #print("accessing feature", self.get_id())
        #traceback.print_stack()
        if self._features is None:
            #print("_features not yet initialized")
            try:
                self.read_features()
                #print("features read from file")
            except (NoFeaturesFileError, FeaturesNotFound,
                    WrongFeaturesFormatError, FeatureParamsError) as e:
                try:
                    #print(e)
                    #print("failed to read features from file, calculating...")
        ...
        #print(self._framesync_features)
    ...
    def select_features(cls, features_id, file_struct, annot_beats, framesync):
        ...
        # If fused feature
        if isinstance(features_id, list):
            for feature in features_id:
                if feature not in features_registry.keys():
                    raise FeaturesNotFound(
                    "The features '%s' are invalid (valid features are %s)"
                    % (feature, features_registry.keys()))

            feats = [features_registry[feat](file_struct, feat_type) for \
                     feat in features_id]            

            return msaf.features.Fuse(file_struct, feat_type, feats)
            

        # Select features with default parameters
        elif features_id not in features_registry.keys():
            raise FeaturesNotFound(
                "The features '%s' are invalid (valid features are %s)"
    ...
    @classmethod
    def get_suitable_fusions(self):
        raise NotImplementedError("This isn't a fused feature type")


    @classmethod
    def compute_file_features(self, file_struct, idx: int):
        print(idx, "Calculating features for", file_struct.audio_file)
        feat_ids = list(msaf.features_registry.keys())
        feat_type = FeatureTypes.est_beatsync
        feats = [msaf.features_registry[feat](file_struct, feat_type) for \
                feat in feat_ids]

        # Compute and write all features
        for feat in feats:
            print(idx, "\t" + feat.get_id() + "...")
            _ = feat.features
