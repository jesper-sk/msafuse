...
class FileStruct:
    def __init__(self, audio_file):
        ...
        self.audio_base = os.path.basename(audio_file)
...
def find_estimation(jam, boundaries_id, labels_id, params):
    ...
    if labels_id is not None:
        ann = ann.search(**{"Sandbox.labels_id": lambda x:
    ...
    for key, val in zip(params.keys(), params.values()):
        #print("205, (%s) %s: %s" % (len(ann), key, val))
        if key == "feature_default" and labels_id is None:
            continue
    ...
    if len(ann) > 1:
        logging.warn("More than one estimation with same parameters.")
...
def save_estimations(file_struct, times, labels, boundaries_id, labels_id,
    ...
    params.pop("features_default", None)
...
def get_configuration(feature, annot_beats, framesync, boundaries_id,
    ...
    config["feature_default"] = \
        feat_def if isinstance(feature, list) and labels_id is not None else None
